...
if __name__ =='__main__':
    if len(sys.argv) < 3:
        ...
        exit(1)
    #input the directory of ground true samples and predict samples
    truth_dict = json.load(open(sys.argv[1], encoding='utf-8'))
    pred_dict = json.load(open(sys.argv[2], encoding='utf-8'))
    acc, _, _, _, Fscore = caculateF(truth_dict, pred_dict)
    print('Avg F1 Score: %f\n' % Fscore)
